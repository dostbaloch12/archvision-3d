...
export default function Process() {
  return (
    <section
      id="process"
      className="scroll-mt-24 bg-gradient-to-b from-[#F4F3F0] to-[#171717] py-24 md:py-32"
    >
      <div className="mx-auto max-w-7xl px-6 md:px-12">
        <div className="grid gap-12 lg:grid-cols-12 lg:gap-16">
          <div className="lg:col-span-4">
            <div className="lg:sticky lg:top-28">
              <p className="text-xs uppercase tracking-[0.2em] text-[#B5AEA3]">Method</p>
              ...
              <p className="mt-6 max-w-sm text-base leading-relaxed text-[#77736B] md:text-lg">
          ...
          <div className="lg:col-span-8">
            {STEPS.map((step, index) => (
              <motion.article
                ...
                className="group relative border-t border-[#D9D6CF] py-10 pl-8 transition-colors duration-500 ease-out hover:border-[#B5AEA3] md:py-14 md:pl-14"
              >
                <span
                  className="absolute left-0 top-10 h-3 w-3 border border-[#BDB9B1] bg-[#F4F3F0] transition-colors duration-500 ease-out group-hover:border-[#B5AEA3] group-hover:bg-[#B5AEA3] md:top-14"
                  aria-hidden="true"
                />

                <div className="flex flex-wrap items-baseline gap-4">
                  <span className="font-serif text-5xl font-light tracking-tight text-[#D9D6CF] transition-colors duration-500 group-hover:text-[#B5AEA3] md:text-6xl">
                    {step.number}
                  </span>
                  <h3 className="text-2xl font-light tracking-tight text-[#171717]">{step.title}</h3>
                </div>

                <p className="mt-5 max-w-xl text-base leading-relaxed text-[#77736B] md:text-lg">
                  {step.copy}
                </p>

                <ul className="mt-6 flex flex-wrap gap-x-6 gap-y-2">
                  {step.outputs.map((output) => (
                    <li key={output} className="text-xs uppercase tracking-[0.16em] text-[#77736B]">
                      — {output}
                    </li>
                  ))}
                </ul>
